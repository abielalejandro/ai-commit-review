...
def _strict_schema() -> str:
    """CommitMessage JSON schema tightened for OpenAI/codex strict mode."""
...
class LlmCodeReviewer(CodeReviewer):
    ...
    def review(
        ...
        return self.llm.with_structured_output(CommitMessage).invoke(messages)
...
class ClaudeCliCodeReviewer(CodeReviewer):
    ...
    def review(
        ...
        schema = json.dumps(CommitMessage.model_json_schema())
        ...
        text = json.loads(out)["result"]  # claude wraps the model output in an envelope
        # ponytail: slice first{..last} instead of a JSON parser that tolerates fences
        return CommitMessage.model_validate_json(text[text.find("{"): text.rfind("}") + 1])
...
class CodexCliCodeReviewer(CodeReviewer):
    ...
    def review(
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            return CommitMessage.model_validate_json(out_path.read_text())
...
class AgyCliCodeReviewer(CodeReviewer):
    ...
    def review(
        ...
        return CommitMessage.model_validate(envelope["structured_output"])
